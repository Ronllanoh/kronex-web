...
export default function MargenBrutoPage() {
  ...
  const margenBruto = precioNeto > 0 ? (utilidad / precioNeto) * 100 : 0;
  const markup = costoNum > 0 ? (utilidad / costoNum) * 100 : 0;
    ...
    <div className="hidden items-center gap-8 text-sm text-slate-300 md:flex">
      ...
      <section className="mx-auto max-w-6xl px-6 py-24">
        ...
        <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
          Calcula cuánto ganas realmente por cada venta según tu costo y precio
          de venta.
        </p>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8">
            ...
            <div className="mt-8 space-y-5">
              <div className="rounded-2xl bg-slate-950 p-5">
                <p className="text-sm text-slate-400">Precio neto</p>
                <p className="mt-2 text-3xl font-bold">{pesos(precioNeto)}</p>
              </div>

              <div className="rounded-2xl bg-slate-950 p-5">
                <p className="text-sm text-slate-400">Utilidad bruta</p>
                <p className="mt-2 text-3xl font-bold">{pesos(utilidad)}</p>
              </div>

              <div className="rounded-2xl bg-slate-950 p-5">
                <p className="text-sm text-slate-400">Margen sobre costo</p>
                <p className="mt-2 text-3xl font-bold">{porcentaje(markup)}</p>
              </div>

              <div className="rounded-2xl bg-cyan-400/10 p-5">
                <p className="text-sm text-cyan-200">Margen bruto</p>
